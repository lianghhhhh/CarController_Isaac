...
def preprocess_planning_data(data):
    current_pos = data[2:5]  # pos_x, pos_y, angle
    temp_data = []
    
    for i in range(10): # 10 target points
        target_pos = data[5 + i*3: 8 + i*3]  # target_x_i, target_y_i, target_angle_i
    ...
    for i in range(10, 20): # 10 obstacles according to target points
        obs_flag = data[5 + i*3]
        obs_x = data[6 + i*3]
        obs_y = data[7 + i*3]
        if obs_flag == 1.0: # have obstacle, calculate relative position
            delta_x = obs_x - current_pos[0]
            delta_y = obs_y - current_pos[1]
            local_dx = (delta_x * np.cos(-current_pos[2]) - delta_y * np.sin(-current_pos[2]))
            local_dy = (delta_x * np.sin(-current_pos[2]) + delta_y * np.cos(-current_pos[2]))
            temp_data.append(obs_flag)
            temp_data.append(local_dx)
            temp_data.append(local_dy)
        else: # no obstacle, use flag and large distance to indicate
            temp_data.append(obs_flag)
            temp_data.append(100.0)
            temp_data.append(100.0)

    data = data[0:2] + temp_data  # vel_left, vel_right + delta states + obstacle info

    return np.array(data).reshape(1, -1)

def preprocess_generate_data(data):
    current_pos = data[2:5]  # pos_x, pos_y, angle
    temp_data = []
    
    for i in range(20): # 20 target points
        target_pos = data[5 + i*3: 8 + i*3]  # target_x_i, target_y_i, target_angle_i
    ...
    for i in range(20, 30): # 10 obstacles according to target points
        obs_flag = data[5 + i*3]
        obs_x = data[6 + i*3]
        obs_y = data[7 + i*3]
        if obs_flag == 1.0: # have obstacle, calculate relative position
            ...
            temp_data.append(obs_flag)
            temp_data.append(0.0) # use 0.0 for no obstacles
